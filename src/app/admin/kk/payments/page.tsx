...
export default function AdminPayments() {
  const pendingPayments = MOCK_ORDERS.filter(o => o.status === 'PAYMENT_CONFIRMATION');
            ...
            <div key={order.id} style={{ background: 'white', border: '1px solid var(--line)', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
              <div style={{ padding: 20, borderBottom: '1px solid var(--line)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--muted)' }}>{order.id}</span>
                  <span style={{ fontWeight: 700 }}>€{order.totalPrice}</span>
                </div>
                <p style={{ fontWeight: 600 }}>{order.buyerName}</p>
              </div>
